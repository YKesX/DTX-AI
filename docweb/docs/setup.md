...
- Optional: NVIDIA GPU with CUDA driver for deep-model training (TabNet, CNN, Bi-LSTM, LSTM-AE — CPU works too, just slower)
...
```bash
# Boot API + dashboard + replay the leakage-safe demo holdout
# (the last 20% of every contiguous fault run, purge-gapped from training)
bash scripts/run_demo.sh

# Grouped replay: complete episodes/runs are held out together
...
Alternatively, start the demo straight from the dashboard: the AI Validation page's **Demo Control** panel lets you choose Dataset demo vs IRL demo (ESP32), pick the model/split/count, and start/stop the run — no terminal needed.

For the IRL demo with real sensors (ESP32 + DS18B20 + BMP280), see the [Hardware Demo](/docs/hardware-demo) page. To wire Isaac Sim itself in, see [docs/isaac_sim_integration.md](https://github.com/YKesX/DTX-AI/blob/main/docs/isaac_sim_integration.md) in the repo.
...
Retrains all 7 model families (Random Forest, LightGBM, XGBoost, TabNet, CNN, Bi-LSTM, LSTM-AE) on the single fixed canonical split — per-episode temporal with a 60-row purge gap — with class weights and early stopping. GPU is used automatically for the torch models if available. Artifacts are written in place under `services/ai/ai/models/`:

- `random_forest/best_rf.pkl`, `lightgbm/best_lgbm.pkl`, `xgboost/best_xgb.pkl`, `tabnet/best_tabnet.zip`, `cnn/best_cnn.pth`, `bilstm/best_bilstm.pth`, `lstm_ae/best_lstmae.pth`
- A fresh `metadata.json` next to each artifact, plus a per-family `scaler.pkl` for the scaled families
- `shared/scaler.pkl` (fallback, always refreshed), `shared/feature_order.json`, `shared/leaderboard.json`, `shared/sanity_baselines.json`, and the winner checkpoint `shared/model_best.pth`

The training notebook `services/ai/dtxai_model_training.ipynb` mirrors the script cell-for-cell and is generated by `python scripts/gen_training_notebook.py` — regenerate it after changing the script instead of hand-editing.
...
At the time of the latest docs refresh, the suite passes locally at `56 passed`.
