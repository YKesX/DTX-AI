...
1. A client (seed script, demo seeder, or dataset replayer) sends a sensor event to FastAPI via `POST /events/`
...
┌──────────────────────────────────────────────────────────────────┐
│  Data Sources                                                    │
│  scripts/seed_demo_events.py  ──► POST /events/  (HTTP)         │
│  scripts/replay_dataset_demo.py ──► POST /events/  (HTTP)       │
│  [future] ESP32 / hardware sensors ──► POST /events/  (HTTP)    │
└──────────────────────────────┬───────────────────────────────────┘
                               │ HTTP :8000
┌──────────────────────────────▼───────────────────────────────────┐
...
|---|---|---|---|
| Backend framework | FastAPI | ≥ 0.111 | REST API + WebSocket server |
| ASGI server | uvicorn[standard] | ≥ 0.29 | Async server |
| Database driver | aiosqlite | ≥ 0.19 | Async SQLite |
| Schema validation | Pydantic v2 | ≥ 2.7 | Request/response models |
| ML — trees | LightGBM | ≥ 4.3 | Anomaly classification |
| ML — trees | XGBoost | ≥ 2.0 | Anomaly classification |
| ML — trees | scikit-learn RF | ≥ 1.4 | Anomaly classification |
| ML — deep | PyTorch | implicit | LSTM Autoencoder |
| XAI | SHAP | ≥ 0.45 | Feature attribution |
| Data processing | pandas / numpy | ≥ 2.0 / 1.26 | Feature engineering |
...
| Shared schemas | dtx-ai-shared | 0.1.0 | Inter-service contracts |
